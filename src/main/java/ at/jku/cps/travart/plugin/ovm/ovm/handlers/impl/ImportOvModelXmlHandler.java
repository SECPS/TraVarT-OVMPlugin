...
public class ImportOvModelXmlHandler extends TransformationHandlerBase<IOvModel, IFeatureModel> {
    ...
    protected void configureFileDialog(FileDialog fileDialog) {
        ...
        String[][] filter = this.getFilter(FMFormatManager.getInstance().getExtensions());
    ...
    protected String[][] getFilter(List<? extends IPersistentFormat<IFeatureModel>> formatExtensions) {
        ...
        for (IPersistentFormat<?> format : formatExtensions) {
            if (format.supportsWrite()) {
                countWritableFormats++;
            }
        }
        String[][] filterArray = new String[2][countWritableFormats];
        String[] names = filterArray[0];
        String[] extensions = filterArray[1];
        int index = 0;
        for (IPersistentFormat<?> format : formatExtensions) {
            if (format.supportsWrite()) {
    ...
    protected IFactory<IFeatureModel> getExportFactory(Path path) {
        ...
        FMFactoryManager factoryManager = FMFactoryManager.getInstance();
        try {
            factory = factoryManager.getFactory(this.getExportPersistance(path));
    ...
    protected FileHandler<IFeatureModel> getExportFileHandler(Path path) {
        FileHandler<IFeatureModel> fileHandler = new FileHandler<>(path, this.getExportFactory(path).create(), this.getExportPersistance(path));
        return fileHandler;
    }
    ...
    protected IPersistentFormat<IFeatureModel> getExportPersistance(Path path) {
        List<IPersistentFormat<IFeatureModel>> formats = FMFormatManager.getInstance().getFormatListForExtension(path.getFileName());
        IPersistentFormat<IFeatureModel> format = formats.isEmpty() ? null : formats.get(0);
        return format;
    }
